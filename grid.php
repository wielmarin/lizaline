<!--- Grid ---->
<div id="grid-container">
	<!-- linker deel grid -->
		<!-- Boven -->

			<div id="grid-left-top-left" class="grid-box smallsquare hoverunderline">
				<p>NEW</p>
			</div>
			<div id="grid-left-top-middle" class="grid-box smallsquare hoverunderline">

				<span id="grid-ltm-description" class="grid-description"><p>White Shoe Rack With Soft Seat - £65.10</p></span>
				<div id="cta-grid">View product</div>
				
			</div>
			
			<div id="grid-left-top-right" class="grid-box smallsquare hoverunderline">
				<?php get_template_part('newmarker'); ?>
				<span id="grid-ltm-description" class="grid-description"><p>Brown Shoe Rack With Soft Seat - £65.10</p></span>
				<div id="cta-grid">View product</div>
				
			</div>		

	<div id="grid-right" class="grid-box largesquare">
		<p class="grid-right-title title-flexbox">Handmade</p>
		<p class="grid-right-subtitle title-flexbox">Wall shelves</p>
		<p class="grid-right-link title-flexbox">Explore</p>
	</div>

			<div id="grid-left-bottom-left" class="grid-box smallsquare hoverunderline">

				<span id="grid-ltm-description" class="grid-description"><p>Decorative light box - £54.90</p></span>
				<div id="cta-grid">View product</div>
				
			</div>
			
			<div id="grid-left-bottom-middle" class="grid-box smallsquare hoverunderline">
				<?php get_template_part('SALEMARKER3'); ?>
				<span id="grid-ltm-description" class="grid-description"><p>Wooden tray - £xx</p></span>
				<div id="cta-grid">View product</div>
				
			</div>

			<div id="grid-left-bottom-right" class="grid-box smallsquare hoverunderline">

				<span id="grid-ltm-description" class="grid-description"><p>Wooden round stool - £49.00</p></span>
				<div id="cta-grid">View product</div>
				
			</div>

	


	


</div>


<!-- End Grid -->